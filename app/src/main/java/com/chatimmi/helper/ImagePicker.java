...
public final class ImagePicker {
    ...
    public static void pickImage(Activity activity) {
        pickImage(activity, "Choose an image");
    }
    ...
    public static void pickImage(Fragment fragment) {
        pickImage(fragment, "Choose an image");
    }
    ...
    public static void pickImageFromCamera(Activity activity) {
        if (!appManifestContainsPermission(activity, Manifest.permission.CAMERA) || hasCameraAccess(activity)){
            Intent intent = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
            intent.putExtra("return-data", true);
            intent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
            ...
            Uri uri = null;
            if (activity != null) {
                uri = FileProvider.getUriForFile(activity, activity.getApplicationContext().getPackageName() + ".fileprovider", getTemporalFile(activity));
            }
            intent.putExtra(MediaStore.EXTRA_OUTPUT,uri);
    ...
    public static void pickImageFromCamera(Activity activity, int FACING) {
        if (!appManifestContainsPermission(activity, Manifest.permission.CAMERA) || hasCameraAccess(activity)){
            Intent intent = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
            intent.putExtra("return-data", true);
